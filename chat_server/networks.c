...
int tcpClientSetup(char * serverName, char * port, int debugFlag)
{
    ...
    // SOCKSTREAM = TCP, its the type of communication
    // 0 - protocol, 0 is default
    // output: file descriptor to magange network I/O
...
int selectCall(int socketNumber, int seconds, int microseconds, int timeIsNotNull)
{
	...
	if ((numReady = select(socketNumber + 1, &fileDescriptorSet, NULL, NULL, NULL)) < 0)
	{
		perror("select");
		exit(-1);
    }
...
void send_header(char flag, int socket_number)
{
    ...
    uint8_t f = flag;
    char buf[HEADER];
    uint16_t length = htons(HEADER);
    memcpy(buf, &length, 2);
    memcpy(&(buf[2]), &f, 1);

    if((sent = send(socket_number, buf, HEADER, 0)) < 0)
    {
        perror("send call, in send_header");
        exit(-1);
    }
    if(sent != HEADER)
    {
        perror("in send_header, send more than 3 bytes");
        exit(-1);
    }
